...
export async function GET() {
  ...
  console.log('--- Starting Extended Data Seed ---')
  const results: any[] = []
  ...
  for (const b of brandsData) {
    try {
      ...
      if (existing.docs.length > 0) {
        ...
        results.push(`Created Brand: ${b.name}`)
      }
    } catch (e: any) {
      console.error(`Failed to create brand ${b.name}:`, e)
      results.push(`Failed Brand: ${b.name} - ${e.message}`)
    }
  }
  ...
  for (const c of categoriesData) {
    try {
      ...
      if (existing.docs.length > 0) {
        ...
        results.push(`Created Category: ${c.title}`)
      }
    } catch (e: any) {
      console.error(`Failed to create category ${c.title}:`, e)
      results.push(`Failed Category: ${c.title} - ${e.message}`)
    }
  }
  ...
  const productsData = [
    ...
    // --- HOSPITAL FURNITURE & PATIENT TRANSFER ---
    {
  ...
  for (const p of productsData) {
    if (!p.brand || !p.category) {
      const msg = `Skipping ${p.name} - missing brand or category ID.`
      console.warn(msg)
      results.push(msg)
      continue
    }
    try {
      ...
      if (existing.docs.length === 0) {
        ...
        const msg = `Created product: ${p.name}`
        console.log(msg)
        results.push(msg)
        addedNew++;
      } else {
        ...
        const msg = `Updated product: ${p.name}`
        console.log(msg)
        results.push(msg)
      }
    } catch (e: any) {
      console.error(`Failed to create/update product ${p.name}:`, e.message)
      results.push(`Failed Product: ${p.name} - ${e.message}`)
    }
  }

  return NextResponse.json({ success: true, addedNew, totalChecked: productsData.length, results })
}
